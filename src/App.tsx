...
const WhatsAppButton = () => (
  <a
    ...
    className="fixed bottom-8 right-8 z-50 bg-[#25D366] text-white p-4 rounded-full shadow-2xl transition-transform hover:scale-110 active:scale-95 flex items-center justify-center"
    id="whatsapp-btn"
  >
...
const Hero = ({ lang }: { lang: Language }) => {
  ...
  useEffect(() => {
    generateTradingImage("A vast soybean plantation at sunset with a modern cargo ship in the background, representing global agricultural export, cinematic lighting, 8k").then(setBgImage);
  }, []);
...
const Logistics = ({ lang }: { lang: Language }) => {
  ...
  useEffect(() => {
    generateTradingImage("A busy commercial port with many colorful containers and a large cargo ship, logistics and international trade concept, sunset").then(setLogImage);
  }, []);
